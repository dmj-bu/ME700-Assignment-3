...
def get_octagon_outline(radius=1.0):
    """
    Return a list of (x, y) coordinate pairs defining a regular octagon.
    The octagon is centered at the origin.
    ...
    -------
    outline_points : list of (float, float)
        List of (x, y) points in counterclockwise order, closing the loop.
    """
    points = []
    for i in range(8):
        angle = 2 * np.pi * i / 8
